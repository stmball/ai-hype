  ...
  return (
    <section className="flex-1 bg-white overflow-y-auto flex flex-col">
      <div className="pt-5 px-6 pb-0">
        <h2 className="m-0 mb-1 text-[1.3rem] font-medium text-gmail-text font-sans">{thread.subject}</h2>
      ...
      <div className="p-4 px-6 flex flex-col gap-3">
        {thread.messages.map((msg, i) => {
          ...
          return (
            <div key={i} className="border border-gmail-border rounded-xl p-4 bg-white">
              <div className="flex items-center gap-[10px]">
                ...
                <div>
                  ...
                  <span className="block text-[0.8rem] text-gmail-muted">{msg.email}</span>
                </div>
                <span className="ml-auto text-[0.78rem] text-gmail-muted whitespace-nowrap">{msg.time}</span>
              </div>
              <div className="mt-3 flex flex-col gap-2 text-[0.92rem] leading-[1.6] text-gmail-text">
                {msg.body.map((p, j) => (
